...
@section('title', 'Quote Requested | Power Codex')
    ...
    <div class="row inbox h-100">
            ...
            <div class="card h-100 left-msg-container">
                <div class="card-body p-0">
                        ...
                        @foreach($inbox as $msg)
                           @php $delay++; @endphp
                            <a href="?email={{$msg->model_email}}&reference={{$msg->model_reference_id}}" >
                                <div class="card left-msg @if($msg->model_status == 0) unread @endif mb-2" data-aos="fade-up" data-aos-delay="{{$delay . '00'}}">
                                    <div class="card-body p-1">
                                        <div class="d-flex align-items-center gap-2 mb-1">
                                            <span class="inbox-avatar">{{substr($msg->model_firstname . ' ' . $msg->model_lastname,0, 1)}}</span>
                                            <div class="d-flex flex-column">
                                                <h6 class="text-capitalize">{{$msg->model_firstname . ' ' . $msg->model_lastname}}</h6>
                                                <div class="d-flex align-items-center gap-2">
                                                    <small>{{$msg->created_at->diffForHumans()}} | </small>
                                                    @if($msg->model_status == 0) <i class='bx bx-envelope'></i> @else <i class='bx bx-envelope-open'></i>@endif
                                                </div>
                                            </div>
                                        </div>
                                        <p class="ellipsis ">{{$msg->model_message}}</p>
                                    </div>
                                </div>
                            </a>
                        @endforeach
        ...
        <div class="col-lg-9">
            <div class="d-flex align-items-center gap-2 mb-2 right-msg-title" data-aos="fade-up" data-aos-delay="200">
                ...
                <h5 class="text-capitalize">{{!empty($messages) ? $messages->model_firstname : 'Messages'}}</h5>
            </div>
            @if(!empty($messages))
            <div class="card right-msg" data-aos="fade-up" data-aos-delay="300">
                <div class="card-body p-2">
                    <div class="d-flex justify-content-between align-items-center">
                        <div class="d-flex align-items-center gap-2">
                            <span class="inbox-right-avatar">{{substr($messages->model_firstname,0, 1)}}</span>
                            <div class="d-flex flex-column">
                                <small>{{$messages->model_email}}</small>
                                <small>{{$messages->created_at->diffForHumans()}}</small>
                            </div>
                        </div>
                        <div class="d-flex flex-column">
                            <small>{{$messages->model_phone}}</small>
                        </div>
                    </div>
                    <p class="my-3">{{$messages->model_message}}</p>
                ...
                @endif
                <form action="{{route('request.reply.send')}}" method="POST" class="d-flex justify-content-between align-items-center gap-2 mt-4">
                    @csrf
                    <input type="hidden" name="ref_id" value="{{$messages->id}}">
                    <input type="hidden" name="email" value="{{$messages->model_email}}">
                    <input type="hidden" name="reference" value="{{$messages->model_reference_id}}">
